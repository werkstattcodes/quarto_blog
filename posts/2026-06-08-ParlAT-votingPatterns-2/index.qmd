...
![Share of comparable bills with a party vote flip, by period, transition and direction](index_files/figure-html/plot-where-flips-heatmap-percent-1.png){group="voting_patterns_2_gallery" fig-alt="Tile heatmap showing, for each party and legislative period, the percentage of comparable bills on which the party changed position, in four panels splitting the two transitions and two directions. Expressed as shares, the committee-to-second-reading row remains dominant."}
...
A note on denominators, since the charts below use more than one. Counts are counts — the first chart simply reports how many changes occurred, which makes periods with more (or more contested) bills look busier for that reason alone. Where a share *is* computed, the denominator is specific to the party it describes: the share-of-bills heatmap divides each party by the bills on which **that party** had a usable vote at both stages, rather than by every bill in the period, so a party that is often absent from the recorded vote lists is not penalised for it. One general point is worth keeping in mind for any of these shares: a party that already voted *yes* at the earlier stage cannot flip *no → yes* — it was never in a position to make that move. Neither of the rates below conditions on this, so both count parties in the denominator that could not have produced the numerator, and both therefore understate how much movement actually occurred. The counts in the first chart are unaffected, since they divide by nothing at all.
...
p_where_bars <- flip_dirs %>%
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      "<span style='font-size:9pt;color:grey40;'>Counts are not normalised for how many comparable bills each period had; see the share-of-bills chart further below for that. NR, regular votes only, GP XX–XXVIII.</span>"
    ),
...
The absolute counts above need the context of how many bills could be compared at each transition. The percentage version below therefore divides, for each party and direction, the number of bills with a flip by the number of bills on which **that party** had a usable vote at both stages of the transition in the same legislative period.

The party-specific denominator matters. An obvious alternative is to divide every party by the same figure — all bills where *any* party could be compared — but that punishes exactly the parties most often missing from the recorded party lists: their numerator shrinks while their denominator doesn't, so their tiles read artificially pale and can't fairly be compared against a party present in nearly every vote. Conditioning each party on its own comparable bills makes the tiles comparable across a row. The percentages still don't sum to 100%: they are independent per-party shares, not a decomposition.
...
#| fig-alt: >
#|   Tile heatmap in a four-panel grid, matching the preceding counts chart but showing
#|   shares instead of counts. Legislative periods XX to XXVIII run along the x-axis and
#|   parties down the y-axis; panel rows separate the two transitions and panel columns the
#|   two directions of change. Each tile gives the percentage of the bills on which that
#|   party was comparable at both stages where it changed position, printed in the tile and
#|   encoded by colour, green for moves toward support and red for withdrawals. Expressing
#|   the flips as shares keeps the committee-to-second-reading row dominant, confirming the
#|   pattern is not an artefact of some periods simply having more bills.
#| fig-width: 12
#| fig-height: 9
#| out-width: "100%"
#| fig-align: "center"
# Denominator, per party: the bills where *this* party was comparable at both stages of the
# transition. `transitions` is already restricted to pairs observed at both stages, so a
# distinct count over it gives exactly that.
transition_bill_totals <- transitions %>%
  distinct(party, item_url, legis_period, transition) %>%
  count(party, legis_period, transition, name = "n_bills")

# Report, rather than assert, which period x transition combinations carry no comparable
# bills at all. A hard equality check on the expected number of combinations would abort the
# whole render over an early period that simply has no committee votes on record.
missing_combos <- expand_grid(
  legis_period = all_periods_vec,
  transition = c("Committee → 2nd reading", "2nd → 3rd reading")
) %>%
  anti_join(
    transition_bill_totals %>% mutate(transition = as.character(transition)),
    by = c("legis_period", "transition")
  )

if (nrow(missing_combos) > 0) {
  warning(
    "No comparable bills for: ",
    paste0(missing_combos$legis_period, " / ", missing_combos$transition, collapse = "; ")
  )
}

stopifnot(all(transition_bill_totals$n_bills > 0))
...
where_tile_rate_grid <- where_tile_grid %>%
  ...
  left_join(
    transition_bill_totals %>% mutate(transition = as.character(transition)),
    by = c("party", "legis_period", "transition")
  ) %>%
  mutate(
...
# The denominator is now per party, so it varies down a column by construction. What must
# still hold is that it is identical across the two *directions* for a given party (both
# directions draw on the same comparable bills) and that it bounds every numerator.
denominator_check <- where_tile_rate_grid %>%
  group_by(party, legis_period, transition) %>%
...
p_where_heatmap_percent <- where_tile_rate_grid %>%
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      "The denominator is per party: the bills where that party itself had a usable vote at ",
      "both stages of the transition, which keeps the tiles comparable across a row. ",
      "Percentages are independent per-party shares and do not sum to 100%. ",
