...
In this post I want to expand the analysis beyond the third reading and include also votes across all stages of the legislative process in the National Council, what includes the first, second as well as the third reading. While doing this my interest is not so much how party voted at a specific stage, but more on whether and *how often party positions on a legislative item actually changed over the course of the different stages within the national Council*. With this general interest a number of other questions lend themselves for answering:
...
- between which stages (committee - first/second reading, first/second - third reading) do the most changes happen?
- during which government we can observe most vote changes?

On a conceptual level, I believe, that answering these questions can contribute to a better understanding of how government parties are responsive to the opposition demands. Strictly speaking, a change of voting, could also simply mean that an opposition party changed its mind after further consideration. While this is possibile, but unlikely, in most cases a change of an opposition party's vote means that the party go something in response. Whether this was indeed a change in the content of the bill at stake, or on another, unrelated issue, can't be answered without looking into the details of the specific item. But retrieving these number is a first step.

A few conceptual notes of caution: - Simply looking at the absolute number of flips can be misleading. For a flip from no to yes materialize we need to have a 'no' in the first place. If there never was a 'no', i.e. if a bill was from the start enjoying unanimous support, there also cannot be any flip. But in this case, the absence of a flip is obviously not indicative for a lack of government responsiveness. - Comparison between legislative periods: Different legislative periods have different legislative items at play, and these may vary in their level of contestation, i.e. how difficult it is for government and opposition parties to find common ground. Not every legislative period brings up items of similar levels of contestation.
...
The image below is from the website of the Parliament and shows the details on the different stages of the one specific bill. it is this information which have retrieved with the code chunk above. 
Two things need to be highlighted. 
1) At this point, we have 
data on **all** stages, not only on voting results. Hence, rows in which we are not interested need to be dropped.
2) Note that the stage data does not contain any info on the voting results inside the committee. This voting data is stored in a different entry, i.e. the entry pertaining to the report of the committee. 


Stages:
https://www.parlament.gv.at/gegenstand/XXVIII/I/449?selectedStage=105

Report:
https://www.parlament.gv.at/gegenstand/XXVIII/I/450?selectedStage=105

How to match them.

To get a better idea of what the code does, let's demonstrate what we did my a concrete example: the latest amendment to the 'Erneuerbaren-Ausbau-Beschleunigungsgesetz und Erneuerbaren-Ausbau-Gesetz'. Querying the item with `get_item_details(stages=TRUE)` retunred the who 
...
item_stages_example_votes <-get_item_details(item_url = "https://www.parlament.gv.at/gegenstand/XXVII/I/2608", votes=T, stages=T)

details_stages_unnest <- item_stages_example_votes %>%
  unnest(stages)


item_stages_example <-get_item_details(item_url = "https://www.parlament.gv.at/gegenstand/XXVIII/I/449", stages=T) %>%
  select(
    item_url,
    legis_period,
    type_doc,
...
item_stages_example_table <- reactable(
  ...
  style_reactable_table(
    ...
    paste0(
      "Every stage row and column for XXVIII/I/449 (Erneuerbaren-Ausbau-Beschleunigungsgesetz), no filtering. Data as of ",
      data_cutoff,
      "."
    )
  )
...
p_flip_no_to_yes <- flip_no_to_yes_grid %>%
  ...
  geom_tile(
    data = flip_no_to_yes_grid %>% filter(is_gov),
...
p_flip_yes_to_no <- flip_yes_to_no_grid %>%
  ...
  geom_tile(
    data = flip_yes_to_no_grid %>% filter(is_gov),
    aes(x = legis_period, y = party),
    fill = NA,
    colour = "black",
    linewidth = 0.4,
...
The two graphs above looked at the step from the second to the third reading. But the procedural journey of an item starts earlier — in the committee (Ausschuss), where a first formal vote on the bill is cast before it ever reaches the plenary floor. With the committee vote now sourced from the reports, we can look at the *earlier* transition: from the committee vote to the second reading. The two graphs below mirror the ones above, only the comparison shifts one stage back.
...
p_flip_comm_no_to_yes <- flip_comm_no_to_yes_grid %>%
  ...
  geom_tile(
    data = flip_comm_no_to_yes_grid %>% filter(is_gov),
    aes(x = legis_period, y = party),
    fill = NA,
    colour = "black",
    linewidth = 0.4,
...
p_flip_comm_yes_to_no <- flip_comm_yes_to_no_grid %>%
  ...
  geom_tile(
    data = flip_comm_yes_to_no_grid %>% filter(is_gov),
    aes(x = legis_period, y = party),
    fill = NA,
    colour = "black",
    linewidth = 0.4,
...
p_where_heatmap <- where_tile_grid %>%
  ...
  labs(
    ...
    subtitle = paste0(
      "Columns: where the flip occurs (committee → 2nd vs 2nd → 3rd). Rows: direction (red = yes → no, green = no → yes). ",
