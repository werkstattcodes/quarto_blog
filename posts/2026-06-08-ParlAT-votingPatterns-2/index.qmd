...
Now to the actual filtering. I first reduce the bills' own second- and third-reading stages down to just the rows where a vote was actually cast — a finished, vote-rows-only dataset for the "ordinary" readings. The committee votes built from the reports above are already vote rows only, by construction (`aub_committee_stages` upstream kept only the adoption-vote stage) — so once both sides are independently down to actual votes, I can simply **bind them together**, and the combined table is the finalized dataset the rest of this post analyzes; no further filtering is needed after the merge. The committee step is the bill's first formal vote, taken before it reaches the plenary floor; including it lets us follow an item across three stages instead of two. Along the way, two new columns are added: `reading` records the stage — committee, second, or third reading (or, in some cases, second and third at once) — and `vote_report_type` captures *how* the vote result is reported in the stage description. The latter matters more than one might think: only stages with an explicit 'Dafür:.../Dagegen:...' pattern (which I label 'regular') state each party's position; other formats such as 'wechselnde Mehrheit' or 'namentliche Abstimmung' don't allow extracting party positions from the stage text alone.
...
Now the merge: `committee_rows` is already vote-rows-only by construction (see `prepare-committee-votes` above — `aub_committee_stages` keeps only the adoption-vote stage), so binding it onto `items_scope_ordinary` needs no further filtering afterward. `vote_report_type` is classified on the combined table, and the result is the finalized dataset the rest of this post analyzes.
