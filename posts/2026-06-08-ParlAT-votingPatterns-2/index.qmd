...
::: {layout-ncol="2"}
![Flip rate by transition, direction and party role, per legislative period](index_files/figure-html/plot-where-flips-bars-1.png){group="voting_patterns_2_gallery" fig-alt="Grouped bar chart in a four-panel grid: legislative periods on the x-axis, flip rate on the y-axis, rows splitting the direction of change and columns splitting opposition from government parties. Blue committee-to-second-reading bars are consistently taller than orange second-to-third-reading bars, and the government column is nearly empty."}
...
::: {.column-margin}
![The committee report's own page, where the adoption vote is recorded. Click to enlarge. [link](https://www.parlament.gv.at/gegenstand/XXVII/I/2666){target="_blank"}](_img/AusschussErneuerbar.png)
:::
...
Across all `r scales::comma(nrow(committee_stages))` committee stages recorded on the bills' own pages, only `r scales::percent(mean(committee_stages$is_vote), accuracy = 0.1)` record a vote at all, and just `r scales::percent(mean(committee_stages$is_adoption_vote), accuracy = 0.1)` are the bill-adoption vote — which is why the committee position is taken from the committee report rather than from these rows. 
...
The most direct answer. For each legislative period, the bars compare the flip **rate** at the two transitions, split into the two directions (no → yes, a move toward support; yes → no, a withdrawal) and by party role.
...
#| fig-alt: >
#|   Grouped bar chart in a four-panel grid. Legislative periods XX to XXVIII run along
#|   the x-axis and the flip rate, conditional on the party-bill pairs at risk of flipping,
#|   on the y-axis. Panel rows separate the direction of change (no to yes, yes to no) and
#|   panel columns separate opposition from government parties. Within each period, a blue
#|   bar shows the committee-to-second-reading transition and an orange bar the
#|   second-to-third-reading transition, with the absolute number of flips labelled above
#|   each bar. The blue bars are consistently the taller of the two, and the government
#|   column is close to empty throughout.
#| fig-width: 11
#| fig-height: 8
#| out-width: "100%"
#| fig-align: "center"
