...
A note on denominators, since the charts below use more than one. Counts are counts — the first chart simply reports how many changes occurred, which makes periods with more (or more contested) bills look busier for that reason alone. Where a share *is* computed, it is computed against the population that could actually produce the numerator: the share-of-bills heatmap divides each party by the bills on which **that party** had a usable vote at both stages, not by every bill in the period. The general point is worth keeping in mind either way: a party that already voted *yes* at the earlier stage cannot flip *no → yes*, so it was never at risk of doing so, and any rate that ignores this overstates how settled positions were.
