...
# actually survive into the analysis (see "Extract votes" further below).
items_in_scope_rel <- items_in_scope %>%
  count(type_doc_long, type_doc, sort = TRUE) %>%
  mutate(rel=n/sum(n))
```

```{r}
#| label: item-type-shares
#| code-fold: false
#| collapse: true
...
committee_vote_rows_table <- reactable(
  ...
  style_reactable_table(
    ...
    paste0(
      "All 'Ausschussberatungen NR' stages containing a vote keyword. 'Adoption vote?' marks the bill-adoption votes used in the analysis. Data as of ",
      data_cutoff,
      "."
    )
  )
...
transitions <- bind_rows(
  ...
  mutate(
    ...
    # who put the item on the table: RV/RVS come from the government, A/BUA
    # originate inside the National Council, the rest from the Bundesrat or
    # are the federal accounts
    origin = case_when(
      type_doc %in% c("RV", "RVS") ~ "Government bill",
      type_doc %in% c("A", "BUA") ~ "National Council",
      .default = "Other"
    ) %>%
      factor(levels = c("Government bill", "National Council", "Other"))
  )
...
The pattern above lumps every item together, but not every item arrives in parliament the same way. A **Regierungsvorlage** is the government's own proposal; a **Selbständiger Antrag** is initiated inside the National Council, usually by MPs of one or more parties. That distinction matters for the question this post started with. If an opposition party votes against a government bill in committee and for it at the second reading, the natural reading is that the government moved to win that support. The same flip on a parliamentary motion can mean something quite different — the motion's own sponsors negotiating among themselves, for instance.

So the table below splits the transitions by who initiated the item, keeping every item type in scope: government bills (`RV`, `RVS`), items initiated in the National Council (`A`, `BUA`), and everything else (`GABR`, `EBR`, `BRA` — Bundesrat initiatives and the federal accounts).

Two things to keep in mind when reading it. First, the rates are **conditional on the pairs at risk**, the same rule as the flip-rate chart further above: the no → yes rate divides by the pairs voting *no* at the earlier stage, the yes → no rate by those voting *yes*. They are therefore *not* comparable with the percentages in the stage × period tile immediately above, which divide by all eligible pairs. Second, `RVS` (treaties) and `BUA` ('Bericht und Antrag') contribute no committee → 2nd pairs at all: treaties are approved with a `Antrag auf Genehmigung des Abschlusses` rather than an adoption vote, and a 'Bericht und Antrag' *is* the committee's own motion, so there is no earlier committee vote to compare it against. For that transition the comparison is effectively `RV` against `A`.

```{r}
#| label: table-origin-flips
#| code-summary: "Flip counts and at-risk rates by item origin, transition and direction"
# at-risk denominator per origin x transition x direction, mirroring the
# conditional rule used by the flip-rate chart above
at_risk_origin <- transitions %>%
  mutate(direction = if_else(from_pos == "Dagegen", "no → yes", "yes → no")) %>%
  count(origin, transition, direction, name = "n_at_risk")

flips_origin <- transitions %>%
  filter(change %in% c("no → yes", "yes → no")) %>%
  count(origin, transition, change, name = "n_flip")

# total comparable pairs, for context on how much each row rests on
pairs_origin <- transitions %>%
  count(origin, transition, name = "n_pairs")

origin_flip_table_data <- at_risk_origin %>%
  left_join(
    flips_origin,
    by = c("origin", "transition", "direction" = "change")
  ) %>%
  replace_na(list(n_flip = 0)) %>%
  mutate(
    rate = n_flip / n_at_risk,
    # short, ASCII-safe suffixes so the pivoted column names stay predictable
    dir_key = if_else(direction == "no → yes", "nty", "ytn")
  ) %>%
  pivot_wider(
    id_cols = c(origin, transition),
    names_from = dir_key,
    values_from = c(n_at_risk, n_flip, rate),
    names_glue = "{.value}_{dir_key}"
  ) %>%
  left_join(pairs_origin, by = c("origin", "transition")) %>%
  arrange(origin, transition) %>%
  select(
    origin, transition, n_pairs,
    n_at_risk_nty, n_flip_nty, rate_nty,
    n_at_risk_ytn, n_flip_ytn, rate_ytn
  )

# a flip can never outnumber the pairs that were at risk of making it
stopifnot(
  all(origin_flip_table_data$n_flip_nty <= origin_flip_table_data$n_at_risk_nty, na.rm = TRUE),
  all(origin_flip_table_data$n_flip_ytn <= origin_flip_table_data$n_at_risk_ytn, na.rm = TRUE)
)

origin_flip_table <- reactable(
  origin_flip_table_data,
  columns = list(
    origin = colDef(name = "Item origin", minWidth = 150),
    transition = colDef(name = "Transition", minWidth = 150),
    n_pairs = colDef(
      name = "Comparable pairs",
      align = "right", minWidth = 120,
      format = colFormat(separators = TRUE)
    ),
    n_at_risk_nty = colDef(
      name = "At risk", align = "right", minWidth = 90,
      format = colFormat(separators = TRUE)
    ),
    n_flip_nty = colDef(
      name = "Flips", align = "right", minWidth = 80,
      format = colFormat(separators = TRUE)
    ),
    rate_nty = colDef(
      name = "Rate", align = "right", minWidth = 90,
      format = colFormat(percent = TRUE, digits = 2)
    ),
    n_at_risk_ytn = colDef(
      name = "At risk", align = "right", minWidth = 90,
      format = colFormat(separators = TRUE)
    ),
    n_flip_ytn = colDef(
  ...
  columnGroups = list(
    colGroup(
      name = "no → yes (move toward support)",
      columns = c("n_at_risk_nty", "n_flip_nty", "rate_nty")
    ),
    colGroup(
      name = "yes → no (withdrawal of support)",
      columns = c("n_at_risk_ytn", "n_flip_ytn", "rate_ytn")
    )
  ),
  fullWidth = TRUE,
  compact = TRUE,
  ...
  style_reactable_table(
    "VOTE FLIPS BY WHO INITIATED THE ITEM",
    paste0(
      "Rates are conditional on the pairs at risk, so they are not comparable with the stage × period percentages above. ",
      "'At risk' counts the party–item pairs holding the prior position, i.e. the only ones that could flip in that direction. Data as of ",
      data_cutoff,
      "."
    )
  )
