...
The object holding that vote is the committee report (`Ausschussbericht`, type `AUB`), which carries the formal `Antrag auf Annahme des Gesetzesvorschlags` section (roughly 'Application for the acceptance of a bill') — this includes each party's vote — plus the reference back to the parent bill we just followed. So to capture committee votes properly, I fetch every committee report for periods 20–28 as well, again with `stages = TRUE`.
...
Now the merge. `committee_rows` is already vote-rows-only by construction (see `prepare-committee-votes` above — `aub_committee_stages` keeps only the adoption-vote stage), and `items_scope_ordinary` has just been reduced to vote rows too. Since both sides are independently down to actual votes, binding them together needs no further filtering afterward. The committee step is the bill's first formal vote, taken before it reaches the plenary floor; including it lets us follow an item across three stages instead of two. `vote_report_type` is classified on the combined table, and the result is the finalized dataset the rest of this post analyzes.
