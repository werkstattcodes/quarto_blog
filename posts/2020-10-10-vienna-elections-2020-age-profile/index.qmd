...
description: "An empirical look at candidates' age."
date: 10-10-2020
...
![](Inkedtext_flow_LI_Moment.jpg){width=50% height=50%}
...
df_clean <- readr::read_csv2(file=here::here("posts","2020-10-10-vienna-elections-2020-age-profile",
                                             "df_clean.csv")) %>% 
  mutate(party=party %>% 
             as_factor() %>% 
             fct_relevel(., sort) %>% 
...
tb_old_young <- df_old_young %>% 
  ...
  cols_width(
      c(index_name) ~ px(100),
      c(year_birth, party, wahlkreis, listenplatz) ~ px(50),
      c(election) ~ px(100)
               ) %>% 
