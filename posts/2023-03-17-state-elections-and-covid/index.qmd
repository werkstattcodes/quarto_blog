...
d\) a few smaller side kicks along the way, including an alternative form of visualizations with a combination of dot- and box-plots (@sec-dotplots).
<br>
...
```{r}
#| code-summary: "Get links to sub/municipality-pages with results."
...
```{r}
#| code-summary: "Define  and apply function to extract election results from subpages"
...
```{r}
#| eval: true
df_res_sbg_raw <- readxl::read_xlsx(path=here::here("posts", "2023-03-17-state-elections-and-covid", "data", "sbg_TW-2018.xlsx"), 
sheet="Stimmen",
...
df_res_sbg_municip_23_fake <- df_res_sbg_municip_18  %>%
mutate(value=value*runif(1,0,2)) %>%
mutate(year=2023)
...
```{r}
df_res_sbg_municip_23 <- df_res_sbg_municip_23_fake %>%
left_join(., df_res_sbg_municip_18 %>%
select(code, type, value),
by=c("code", "type")) %>%
rename(value_23=value.x,
value_18=value.y) %>%
mutate(value_18=replace_na(value_18, 0)) %>% # otherwise: number - NA = NA !!
mutate(value_diff=value_23-value_18) %>%
...
nrow(df_res_sbg_municip_23)
nrow(df_res_sbg_municip_23_fake)
...
## Select parties 
```{r}
#TODO Limit analysis to main parties
...
levels(df_res_municip_covid$party)
...
#| include: false
txt_caption <- "Data: 
https:&#47;&#47;www.ktn.gv.at&#47;wahlen&#47;ltwahl2023; https:&#47;&#47;www.salzburg.gv.at&#47;pol&#47;wahl&#47;land&#47;ltw23; https:&#47;&#47;www.noe.gv.at&#47;wahlen, https:&#47;&#47;info.gesundheitsministerium.gv.at&#47;data&#47;<br>Analysis & Graph: Roland Schmidt | @zoowalk | **https:&#47;&#47;werk.statt.codes**"
```
...
ggplot(., aes(y=change_perc, 
  ...
  linewidth=.5)+
  # geom_rect(data=. %>% filter(str_detect(party, regex("FPÖ|ÖVP"))),
...
#vec_plot <- df_plots %>% filter(state=="noe")  %>% pull("plot")
