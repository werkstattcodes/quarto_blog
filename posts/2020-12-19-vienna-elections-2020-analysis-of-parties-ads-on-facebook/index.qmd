...
description: "Analysis of the facebook ads placed in the run-up to the 2020 Vienna city elections."
date: 12-19-2020
...
format: 
  html:
    ...
    smooth-scroll: true
    highlight-style: ally-dark
    
categories: [Austria, social media, elections]
...
# plot_bg_color <- readr::read_file(file=here::here("styles.css")) %>% 
...
```{r}
#| eval: false
#| code-summary: "Get data from Facebook API"
...
```{r include=FALSE}
df_ads <- readr::read_rds(file=here::here("posts", 
                                          "2020-12-19-vienna-elections-2020-analysis-of-parties-ads-on-facebook",
                                          "df_ads.rds")) 
```
...
```{r}
#| code-summary: "Exclude ads refering to Wiener Neustadt"

df_ads <- df_ads %>% 
  filter(str_detect(ad_creative_body, "Wien(?!.*Neustadt)"))
...
```{r include=FALSE}
pages_of_interest <- readxl::read_xlsx(path=here::here("posts", "2020-12-19-vienna-elections-2020-analysis-of-parties-ads-on-facebook", "df_pages_of_interest.xlsx")) %>% 
  # filter(include=="yes") %>% 
...
rt_pages <- reactable(pages_of_interest,
          ...
          compact=T,
          theme = rt_theme) %>%
          add_title(title="WIEN-WAHL 2020: Facebook pages within scope and affiliated party") %>%
          add_source(source=my_caption)
rt_pages
...
```{r}
#| code-summary: "Code: Data mugging, unnest lists"
...
#           wrap=F,
#           theme=rt_theme) %>%
#           reactablefmtr::add_title(title="WIEN-WAHL 2020: All Facebook ads") %>%
#           sub_title(subtitle="Ads posted between {date_observation_start_format} and {date_observation_end_format}")
...
#readr::write_csv2(df_impression_check,file=here::here("blog_data","2020-12-19-vienna-elections-2020-analysis-of-parties-ads-on-facebook","df_impression_check.csv"))
```
...
```{r}
#| code-summary: "Code: Table on parties' number of ads and total spending"
#| message: false
#| warning: false
...
rt_party_pages %>%
reactablefmtr::add_title(title="WIEN-WAHL 2020: Number and total amount spent by party",
font_size=12) %>%
add_subtitle(subtitle=glue::glue("Click to see details for indiviudal Facebook pages. Ads placed between {date_observation_start_format} and {date_observation_end_format}."), font_size=10) %>%
reactablefmtr::add_source(my_caption, font_size=8)
```
...
#| message: false
#| warning: false
#| code-summary: "Code: Graph on expendigure"
...
```{r message=FALSE, warning=FALSE}
#| code-summary: "Code: Impression of ads"
...
#| message: false
#| warning: false
#| code-summary: "Code: Distribution of ads over impression categories"
...
```{r}
#| code-summary: "Code: Impressions per ad category"
...
```{r}
#| code-summary: "Code: Unnest age list column"
...
```{r}
#| code-summary: "Code: Plot age profile of ads"
#| message: false
#| warning: false
...
```{r}
#| code-summary: "Code: Gender of ads' audience"
#| message: false
#| warning: false
...
```{r}
#| code-summary: "Code: Page with female audience"
#| message: false
#| warning: false
...
rt_female %>%
add_title(title="WIEN-WAHL 2020: Accounts which placed ads with a median female audience share of >50%") %>%
add_source(source=my_caption)
