...
# create 3rd reading indicator (TRUE if there's a match)
items_details <- items_details %>%
  mutate(
...
The last table in the chunk above is informative. Almost all items are either a (third reading AND have votes) OR (do not have votes AND are NOT third readings). There a two items which don't fit the pattern. I checked them an these are items where the second and the third reading were in the _same_ session (hence our search for 'dritte?r Lesung' had a match), but the item failed to cross the second reading (hence not vote.) Based on this result, we can confidently conclude that all our extracted votes pertain to a third reading. Let's unnest the data now to get the actual results.
...
# Classify each voted item as unanimous or split across party groups.
...
p_votes_contested <- votes_contested_shares %>%
  ...
  labs(
    x = NULL,
    y = "Share of voted items",
    title = "Split votes by legislative period",
    subtitle = paste0(
  ...
  theme(
    ...
    plot.subtitle = ggtext::element_textbox_simple(
      size = 10.5,
      lineheight = 1.12,
      margin = ggplot2::margin(t = 3, b = 10)
    ),
...
The above graph was relative crude in the sense, that it only differentiated between unanimous and split votes. In the following step, I calculate the in-favor vote share for every item. Taken together for each legislative period, we can calculate the median, mean, and standard deviation of these vote shares. This should allow us to compare the average support for an item voted at during the thrid reading. 
...
p_support_share_summary <- support_share_avg_plot %>%
  ...
  theme(
    ...
    plot.subtitle = ggtext::element_textbox_simple(
      size = 12,
      lineheight = 1.12,
      margin = ggplot2::margin(t = 3, b = 12)
    ),
    plot.caption = ggtext::element_textbox_simple(
      hjust = 0,
      halign = 0,
...
histogram_support_bin_labels <- paste0(
  scales::percent(head(histogram_support_breaks, -1), accuracy = 1),
  "-",
  scales::percent(tail(histogram_support_breaks, -1), accuracy = 1)
)
...
support_share_histogram <- support_share %>%
  ...
  mutate(
    ...
    support_bin = cut(
      ...
      labels = histogram_support_bin_labels,
      include.lowest = TRUE,
      right = TRUE
    )
  ) %>%
  count(legis_period, support_bin, name = "votes_n") %>%
  mutate(vote_share = votes_n / sum(votes_n), .by = legis_period)

p_support_share_histogram <- support_share_histogram %>%
  ggplot(aes(x = support_bin, y = vote_share)) +
  geom_col(
    colour = "white",
    fill = "#1185FE",
    linewidth = 0.25,
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      "Bar heights sum to 100% within each legislative period."
    ),
    caption = vote_graph_caption
  ) +
  theme_minimal(base_size = 12) +
  theme(
    ...
    strip.text = ggtext::element_markdown(size = 10.5, hjust = 0),
...
## Prepare remaining visualisations

```{r}
#| code-summary: "Prepare data for visualisations"
#| code-fold: true
#harmonize/collapse some party names
items_votes <- items_votes %>%
  mutate(
    text = case_when(
...
## Graphic 3: Relative Frequency of vote type
