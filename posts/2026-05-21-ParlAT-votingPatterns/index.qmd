...
I have been recently tinkering with the `get_item_details()` function of the [ParlAT package](https://werkstattcodes.github.io/ParlAT/index.html "Open ParlAT page"){target="_blank"}. While a few things are still in flux (it’s version 0.0.4 after all), the main development is the inclusion of the `votes` parameter into the function arguments. When set to TRUE the function now returns the voting result of the provided item ('Verhandlungsgegenstand') in its third reading in the National Council ('dritte Lesung im Nationalrat').
...
The `get_item_details()` parses the data which lie behind the graph on the voting results, i.e. it returns the number of MPs per party who votes in favor or against the specific item.
...
As you will see, the querying of the relevant data is rather straightforward. Most of the code presented was needed to create the graphs, so don’t get distracted by some length code chunks.

And before I finally dive into the actual code, the usual caveat apply a) this is work in progress, if you see any error, have suggestion etc. do not hesitate to contact me with feedback ([bluesky direct msg](https://bsky.app/profile/zoowalk.bsky.social){target="_blank"} or the [repo forum](https://github.com/werkstattcodes/ParlAT/discussions){target="_blank"} if directly related to ParlAT); b) although I am developing the ParlAT package, I don’t consider myself as an expert on all the ins-and-outs of the Austrian Parliament. The main motivation is to provide a tool which makes researching the Austrian Parliament a bit easier, and c) this is a blog post and does not pretend to be an academic output etc.
...
Before using our above obtained results to query the voting results, it might be instructive to demonstrate the return value of the new \`vote parameter with a single item. The item is the one for which we saw above the results. Note that the return value for `votes` is a list column, it contains a data frame with nested data.
...
So as you can see from the above chunk, the returned votes column is a list column which contains again the list column `result`. From here, we can extract the party name (`text`) and the vote result (`infavor`). (Note, in a later version the function might get further streamlined so that the return value is already the nested result).
...
With the data now available, let's see whether we can find anything meaningful. As a first step, I want to know how many items were passed on an unanimous vote vs split vote.
...
::: {.callout-important}
HOWEVER, the table below also reveals that the data for the XXVII and XXVII legislative period feature suspiciousyl stable data/most likely wrong data. It seems as if the mode of repoting has changed. This is a significatn limitation when dealing with questions which need the exact count for every party. Wonder what's the reason behind.
:::
...
Bearing in mind tha above caveat for the last two legislative periods, let's continue with the analysis of the in-favor votes. Above I showed the median, but the median is also only a summary statistic and conceals the shape of the underlying distribution. To remedy this, the histogram below puts each vote into a bin corresponding to its vote-in-favor share. The x-axis shows five percentage-point bins of the item's in-favor vote share (e.g. 60-65 %). The y-axis is the relative frequency of the items in the relevant bin within a legislative period. Within a facet, all bars amount to 100 %. The red dotted line marks parliamentary vote share of the governing parties in that legislative period (it's not perfectly aligned, but you get the idea).
...
p_party_vote_shares <- party_vote_shares %>%
  ...
  labs(
    ...
    subtitle = paste0(
      ...
      "Example: In 65 % of all items voted on in the third reading during the XX legislative period, the FPÖ voted against."
    ),
    caption = vote_graph_caption
  ) +
  theme_vote_plot() +
...
What's noteworthy, at least to me, is the high no-ratio of the current FPÖ fraction. It is only exceeded by the FPÖ in the XX legislative period, i.e., during the Haider years, where the party voted against almost two thirds of the items covered here.

There are also interesting opposition differences. In XXVI, JETZT/PILZ and SPÖ voted no more often than NEOS, suggesting a clearer left-opposition profile against the ÖVP-FPÖ government. In XXVII, by contrast, FPÖ and NEOS have similarly high no-vote shares, while SPÖ is somewhat less oppositional on this simple yes/no measure.

## Graphic 5: Party-pairs

If you are still reading, congrats! I hope the remaining graphs will be enough of a reward.

Next, I wanted to take the vote results and see how often one party voted together with another party, and this irrespective if the vote would be in favor or against a particular item. The result is the party-pair matrix below.
...
I believe there's quite a bit in it, and others who have more time and work on this professionally may want to take this up. What stands out to me is again the FPÖ of the current XXVIII and the XX legislative period. The FPÖ votes, compared to other parties but also when compared to itself in other legislative periods, comparatively seldom together with other parties. A bit of a loner.

The other clear pattern is the near-perfect agreement among governing parties. This holds for SPÖ-ÖVP grand coalitions, ÖVP-FPÖ governments, the ÖVP-Greens coalition, and the current ÖVP-SPÖ-NEOS coalition. In other words, coalition membership appears to be much more predictive of voting alignment than broad ideological distance.

The matrix also shows how quickly alignments can change once parties move between government and opposition. SPÖ and ÖVP vote almost in lockstep during grand-coalition periods, but their agreement drops sharply in XXVI once ÖVP governs with FPÖ. That makes the graph useful not only as a measure of ideological proximity, but also as a map of parliamentary roles.
...
Note that the percentages within a legislative period do *not* add up to 100%. This is by definition. E.g. the 30 % in which the FPÖ was the sole party to vote no, we have another 30 % where all other parties voted yes. Hence, the same items show up two times in the graph.
...
The UpSet plot makes the same basic story more concrete because it shows the actual party combinations behind the aggregate shares. In XXVIII, the two largest patterns are especially telling: FPÖ-alone no votes and government-plus-Greens yes votes. This indicates that the current opposition is not symmetrical; the FPÖ is much more often isolated in the no camp than the Greens.

Government-only yes coalitions are another recurring pattern. In XXI and XXII this means ÖVP-FPÖ, in XXVI again ÖVP-FPÖ, and in XXVIII ÖVP-SPÖ-NEOS. These bars are the clearest visual counterpart to the support-share histograms: they are the items that pass with the governing majority rather than with broader parliamentary backing.

Finally, the in-favor and not-in-favor sides should be read together. A vote where only the FPÖ says no will usually produce a corresponding yes-combination made up of all other parties. That is why the percentages within a legislative period do not add up to 100 percent; the graph is showing both sides of the same roll-call structure.
