...
agreement_matrix <- df_divided |>
  ...
  inner_join(
    ...
    by = "item_url", relationship = "many-to-many"
  ) |>
  filter(party != party2) |>
...
df_win_rate <- df_divided |>
  mutate(on_winning_side =
    (vote_result == "passed"   & vote == "for") |
...
ggplot(df_win_rate, aes(x = pct_wins, y = party)) +
  geom_segment(aes(x = 0, xend = pct_wins, yend = party),
               colour = "grey75", linewidth = 1) +
  geom_point(aes(colour = pct_wins), size = 5) +
  geom_text(aes(label = scales::percent(pct_wins, accuracy = 1)),
            hjust = -0.5, size = 3.5) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1),
                     limits = c(0, 1.18),
  ...
  labs(
    x = "Share of contested votes on the winning side", y = NULL,
...
fpoe_alignment |>
  ...
  mutate(
    alignment = factor(alignment,
      levels = c("FPÖ alone", "One party agrees", "Majority agrees", "All others agree")),
    pct = n / sum(n)
  ) |>
  ggplot(aes(x = pct, y = alignment, fill = alignment)) +
  geom_col(width = 0.6, show.legend = FALSE) +
  geom_text(aes(label = paste0(scales::percent(pct, accuracy = 1), " (n=", n, ")")),
            hjust = -0.1, size = 3.5) +
  scale_x_continuous(labels = scales::percent_format(accuracy = 1),
