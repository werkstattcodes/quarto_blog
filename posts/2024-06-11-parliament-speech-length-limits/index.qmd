...
## On the respect of speech time limits {.unnumbered}

::: {layout-ncol=3}

![](images/n_speeches_party.png){group='gallery'}

![](images/limits.png){group='gallery'}

![](images/n_speeches_overtime_limit_type.png){group='gallery'}

![](images/top_5_overtime_speeches.png){group='gallery'}

![](images/mp_overtime_cum.PNG){group='gallery'}

![](images/overtime_cum.PNG){group='gallery'}

![](images/overtime_speech_time.png){group='gallery'}

![](images/n_speeches_overtime.png){group='gallery'}

![](images/overtime_party.png){group='gallery'}

![](images/overtime_categories.png){group='gallery'}

:::


## On legislative periods in comparison {.unnumbered}


::: {layout-ncol=3}

![](images/legis_comp.PNG){group="gallery_period"}

![](images/nr_duration_session.png){group="gallery_period"}

![](images/relative_frequency_sessions.png){group="gallery_period"}

![](images/cum_n_speeches.png){group='gallery'}
...
Council (for previous posts see e.g. [here](https://werk.statt.codes/posts/2023-01-18-parliament-new-api-mps/){target='_blank'} and [here](https://werk.statt.codes/posts/2021-04-22-how-often-do-austrias-chancellors-use-the-word-i/){target="_blank"}).
...
to look into, I took a few detours and
...
multiple times in the dataframe. This has to be accounted for. To get a better idea, below some sample
rows.
...
With the data on speeches per legislative sessions available, there are already
a few things which we can have a look at.
...
When compiling the dataset, I noticed that the XVIII legislative period
...
the XVIII legislative period, i.e. there is a considerable chuck of
...
```{r}
#| code-summary: "Number of sessions and duration of legislative period"
...
In the next "detour" - remember this post is actually meant to be about excessive
...
parallel has its limits. But it's nevertheless noteworthy, and you
...
[^1]: For a (not entirely serious) analysis of government speeches see [here]("https://werk.statt.codes/posts/2021-04-22-how-often-do-austrias-chancellors-use-the-word-i/"){target="_blank"}.
...
mandate, we can identify which speeches were (not) by an MP. With this approach,
we also include speeches of an individual who was at one point an MP, but exclude the speeches of the same individual if he/she later became e.g. a member of government.
...
tbl_speeches_party %>%
  ...
  add_title(title = html("Austrian National Council (Nationalrat):<br>XXVII Legislative Period: Number of Speeches in the Plenary by Party Affiliation"), font_size = 14) %>%
...
Generally, at least that's how I got it, speeches' time limits are subject to a
...
between, on the one hand, formal time limits governing the maximum speech length in different debate types, and on the other hand, voluntary time limits. As it becomes clear, the practice of reduced, voluntary time limits is rather common.
...
"Begründung eines Dringlichen Antrags in einer Plenarsitzung". AFAIK,
...
applicable time limits was available. 
...
The graph below shows the five speeches by MPs with the longest overtime
...
all `r nrow(df_session_speeches_27_mps) %>% scales::number(., big.mark=',', decimal.mark='.')` speeches* held so far (hence the page may take a bit longer to load). You can search by party, speaker etc. and
...
speech one minute earlier than she/he would have been obliged to, then
...
The graph below is essentially a simplified visualization of some of the
data contained in the table above. 
...
txt_subtitle <- glue::glue("Each dot represents an MP. The position on the x-axis represents the total time of his/her speeches in the plenary of the National Council in the course of the XXVII legislative period (as of {lubridate::today() %>% format(., '%d.%m.%Y')}). The position on the y-axis indicates the total overtime of these speeches. There is a group of FPÖ MPs who not only feature a large total speech time, but also larger overtime, even if compared to their peers who feature somewhat similar total speech times. Neos MP Loacker stands out not only with the longest total speech time, but also with a relatively moderate total overtime, considering that no one spoke longer than him.")
...
Data covers all speeches held by MPs in the plenary of the National Council during the XXVII legislative period as of {lubridate::today() %>% format(., '%d.%m.%Y')}. Difference of sum of labels to total number is due to incomplete data for {df_abs %>% filter(is.na(speech_overtime_cat)) %>% pull(speeches_n_cat) %>% unique()} speeches.")
...
txt_subtitle <- glue::glue("The graph shows the relative number of plenary speeches by MPs per party and per speeches' overtime. Altough relatively small in absolute terms, there is a striking increase in the share of speeches by FPÖ MPs the larger the overtime gets. Data on {df_rel %>% filter(is.na(speech_overtime_cat)) %>% summarise(N_na=sum(speeches_n)) %>% pull(N_na)} speeches missing.")
...
Well, my blog post gets again much longer than initially intended. Furthermore, and more importantly, since the founding of the 2nd Republic (legislative period V), there has been no legislative period with more sessions of the National Council than the current XXVII (which is even not yet concluded at the time of writing).

When it comes to the number of speeches made during a legislative period, the analysis is somewhat limited due to the lack of data on individual speeches prior to legislative period XIX. Remarkably, though, with the data available, we can see that during the hight of the pandemic the National Chamber was in no way stymied in its deliberations as the high number of speeches by MPs and others demonstrates.

When it comes to the initial interest of this post, speech limits, we saw that the vast majority of speeches are governed by voluntary time limits. However, only a minority, around 36 % of these speeches, de facto stay within their limit. In other words, exceeding these limits and having a speech overtime is rather prevalent across all parties (with the exception of the non-affiliated MP Pia Beck).

Nevertheless, MPs of the FPÖ stand out for their overtime. The FPÖ/its MPs feature prominently when it comes to 1) individual speeches with the largest overtime (6 out of the top 10), 2) MPs with the largest aggregated overtime (7 out of the top 10), and 3) the party as a whole features the largest overtime when considering their total speech length (18 % of total speech time). I am somewhat tempted to make a general statement about 'anti-system' parties and the non-respect for (soft) norms, but based on this single date point I leave this for the category 'further research...'.

If you made it all the way to the end of this post - congrats, and thanks for reading.

One last thing - I am doing more and more freelance work these days. If you like this post and think that such work (on similar or different topics) can be helpful to you, let's talk. You can reach me best via [mastadon](https://mastodon.social/@zoowalk){target="_blank"} or X
